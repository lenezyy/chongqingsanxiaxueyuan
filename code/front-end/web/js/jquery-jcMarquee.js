...
;(function($){
	$.fn.jcMarquee = function(options) {
		...
		return this.each(function() {
	        var $marquee = $(this);
			$marquee.append("<ul class='clone'>"+"</ul>");
			$marquee_ul = $marquee.find('ul').eq(0);
			$marquee_ul.children().clone().appendTo('.clone');
			var $marquee_left = $marquee.find('li');
			if (options.marquee == 'x') {
				var x = 0;
		        $marquee.css('width','1000%');
		        $marquee.css('position','relative');
				$marquee_left.css({'margin-right':options.margin_right});
                function Marquee_x(){
					//console.debug(x);
	                //$marquee.scrollLeft(++x);
					$marquee.css('left',--x+'px');
		            if(-x==$marquee_ul.width()) { x = 0 };
				};
			...
			if (options.marquee == 'y') {
				var y = 0;
		        $marquee.css('height','1000%');
				$marquee_left.css('margin-bottom',options.margin_bottom);
                function Marquee_y(){ 
	                $marquee.scrollTop(++y);
		            if(-y==$marquee_left.height()) { y = 0 };
				};
